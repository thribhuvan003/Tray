...
- **Custom Showcase Previews Calibrations (`PiranhaPortalsSection.tsx`)**:
  ...
  - Kitchen card frame calibrated to a highly legible tablet-optimized view (`virtualWidth = 800`, `scrollPx = 0`). This focuses directly on the central tickets queue, hiding sidebars and scaling the active ticket columns for double font size readability, while retaining the top header bar, clock, and canteen dropdown selectors.
