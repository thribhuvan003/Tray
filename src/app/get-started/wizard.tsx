...
export function GetStartedWizard({ isNewUser = false, isSignedIn = false }: { isNewUser?: boolean; isSignedIn?: boolean } = {}) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      // Sign in the browser with the new/existing account so the session cookie
      // is established before we navigate to the admin dashboard.
      const loginTarget = `/c/${result.canteenSlug}/admin/dashboard?welcome=1`;
      const loginFallback = `/c/${result.canteenSlug}/login?role=owner&next=${encodeURIComponent(loginTarget)}`;
      try {
        ...
        if (loginError) {
          // Session not established — send to login page which will route correctly
          window.location.href = loginFallback;
          return;
        }
      } catch {
        // Any exception during sign-in → fallback to login page
        window.location.href = loginFallback;
        return;
      }
