...
// La Revoltosa-inspired fullscreen wipe transition.
// Used only when clicking "Open student app", "Kitchen view" etc. in the
// try-demo role selector. Plays once, then navigates.
//
// Design: cream overlay fills from bottom → huge "Tray" wordmark → role label
// → palette wipe fades out → page opens.
//
// Respects prefers-reduced-motion (skips the transition entirely if set).
...
export function DemoEntryTransition() {
  ...
  useEffect(() => {
    ...
    // After the wipe in, pause briefly then navigate
    const t1 = setTimeout(() => {
      setState("leaving");
    }, 700);
    const t2 = setTimeout(() => {
