...
const STATUS_META: Record<string, { label: string; icon: typeof Clock; color: string; pulse: boolean }> = {
  ...
  payment_failed:  { label: "Payment failed",   icon: Clock,        color: "text-rose-500",    pulse: false },
};
...
export function StudentOrdersView({
  ...
  useEffect(() => { setOrders(initialOrders); ordersRef.current = initialOrders; }, [initialOrders]);
      ...
      .on(
        ...
        (payload) => {
          ...
          const statusFromEvent: Record<string, string> = {
            ...
            rejected: "rejected", expired: "expired", cancelled_by_student: "cancelled_by_student",
            cancelled_by_kitchen: "cancelled_by_kitchen", refunded: "refunded",
          };
          const newStatus = ev.event_type === "status_changed" ? (ev.payload?.to as string) : statusFromEvent[ev.event_type];
          if (newStatus) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 pt-8 pb-20">
      ...
      {activeOrders.length > 0 && (
        <section className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.16em] text-ocean-500">Active orders · live</span>
          </div>
          <div className="flex flex-col gap-3">
            {activeOrders.map((o) => {
              ...
              const href = o.status === "pending_payment" ? `/c/${tenantSlug}/pay/${o.id}` : `/c/${tenantSlug}/track/${o.id}`;
              return (
                <Link key={o.id} href={href}
                  ...
                  <div className="h-0.5 bg-[color:var(--color-line)] w-full">
                    <div className="h-full bg-ocean-500 transition-all duration-700 ease-out" style={{ width: `${progress}%` }} />
                  </div>
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`h-11 w-11 shrink-0 rounded-xl bg-ocean-50 dark:bg-ocean-500/10 inline-flex items-center justify-center ${meta.pulse ? "animate-pulse" : ""}`}>
                          <Icon size={20} className={meta.color} />
                        </div>
                        <div className="min-w-0">
                          <div className="text-[15px] font-semibold tabular text-[color:var(--color-ink)]">{o.short_code}</div>
                          <div className="text-[12px] font-mono text-[color:var(--color-ink)]/50 mt-0.5">{formatTimeIST(o.placed_at)}</div>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-[15px] font-bold tabular text-[color:var(--color-ink)]">{formatRupees(o.total_paise)}</div>
                        <span className={`inline-block mt-1 text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full ${
                          o.status === "ready" ? "bg-emerald-500/15 text-emerald-600 border border-emerald-500/30"
                            : o.status === "preparing" || o.status === "partially_ready" ? "bg-amber-500/15 text-amber-600 border border-amber-500/30"
      ...
      {pastOrders.length > 0 && (
        <section>
          {activeOrders.length > 0 && (
            <div className="text-[10px] font-mono uppercase tracking-[0.16em] text-[color:var(--color-ink)]/45 mb-3">Past orders</div>
          )}
          <ul className="flex flex-col gap-2">
            {pastOrders.map((o) => {
              ...
              return (
                <li key={o.id}>
                  <Link href={`/c/${tenantSlug}/track/${o.id}`}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-[13.5px] font-medium truncate">{o.short_code}</div>
                    ...
                    <div className="text-right shrink-0">
                      <div className="text-[13.5px] font-semibold tabular">{formatRupees(o.total_paise)}</div>
                      <span className={`inline-block mt-1 text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full ${isGood ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-500"}`}>
                        {meta.label}
                      </span>
                    </div>
                  </Link>
      ...
      {orders.length === 0 && (
        <div className="mt-10 rounded-2xl border border-dashed border-[color:var(--color-line)] p-12 text-center">
          ...
          <Link href={`/c/${tenantSlug}/menu`}
            className="mt-4 inline-flex items-center gap-1.5 h-11 px-5 rounded-full bg-ocean-500 text-white text-[13px] font-medium hover:opacity-90 transition-colors">
            Browse the menu →
          </Link>
        </div>
      )}
