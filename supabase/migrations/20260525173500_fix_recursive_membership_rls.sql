...
CREATE POLICY "Users can view members of same tenant" ON public.tenant_memberships
  FOR SELECT
  USING (
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'memberships') THEN
    ...
    EXECUTE 'CREATE POLICY "Users can view members of same tenant" ON public.memberships FOR SELECT USING (tenant_id = ((auth.jwt() -> ''user_metadata'' ->> ''tenant_id'')::UUID))';
  END IF;
END $$;
